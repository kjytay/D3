...
// ADD CHART
$("#stock .chart").sparkline(  // barchart for volume
	$.map(stock, function (wk) { return wk.volume; }),
	{
		...
		height: "60px",
		barWidth: 400 / stock.length,
		defaultPerPixelValue: 5
	}
).sparkline(  // chart for closing price
	$.map(stock, function (wk) { return wk.adj_close; }),
	{
		composite: true,  // don't erase existing chart but draw over it
		lineColor: "#006363",
		// alpha set to 0.3 so that vol chart can still be seen
		fillColor: "rgba(45, 153, 153, 0.3)",
		disableTooltips: true
	}
).on("sparklineRegionChange", function (ev) {
	/* we get idx from 2nd element. This is because 1st element of the
	   array is the 1st chart; sparklines doesn't return useful info
	   from bar charts, so we need to get from line chart */
	...
	if (idx) {
		$(".info").html(
		  ...
		  + "&nbsp;&nbsp;&nbsp; Volume: "
		  + (Math.round(stock[idx].volume/10000)/100).toFixed(2) + "M" );
		  
		$(".details").html(
			"Open: $" + stock[idx].open.toFixed(2) + "<br/>"
		  + "High: $" + stock[idx].high.toFixed(2) + "<br/>"
		  + "Low: $"  + stock[idx].low.toFixed(2) );
		
		// make a boxplot of sorts
		$(".details").sparkline([
			stock[idx].low,
			...
			type: "box",  // boxplot
			showOutliers: false,
			// if close > open, green. if not, red
