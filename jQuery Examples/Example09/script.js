...
// ADD CHART
$("#stock").sparkline(  // line chart for closing price
...
).on("sparklineClick", function (ev) {  // to handle a click event
	var sparkline = ev.sparklines[0],
	region = sparkline.getCurrentRegionFields();
	// region.x & region.y are coordinates of the click
	
	// bring up the widget & x button, hide stock chart
	$("#widget").slideDown();
	$("#widget-control").slideDown();
	$("#stock").SlideUp();
});

$("#widget-control").click(function (ev) {
	// override default event handling of jumping to top of page
	ev.preventDefault();
	// bring up stock chart, hide widget & x button
	$("#widget").slideUp();
