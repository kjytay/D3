...
$("#stock .chart").sparkline(
	$.map(stock, function (wk) { return wk.adj_close; }),
	{
		...
		minSpotColor: "#CA0000",
		maxSpotColor: "#CA0000",
		disableTooltips: true  // to disable the default tool tip
	}
);

$(".chart").on("sparklineRegionChange", function (ev) {
	var idx = ev.sparklines[0].getCurrentRegionFields().offset;
	/* If it's defined, idx has the index into the data array
	corresponding to the mouse pointer */
	
	// if defined, put details into the "info" div
	if (idx) {
		$(".info").html(
...
}).on("mouseout", function () {  // remove info when out of chart area
	$(".info").html("&nbsp;");
	$(".details").html("");
});